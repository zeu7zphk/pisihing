...
local freezePlayerRemote = replicatedStorage:WaitForChild("FreezePlayerRemote")
...
-- Função para solicitar ao servidor que aplique a lentidão no jogador
local function slowPlayer(targetPlayer)
    if targetPlayer then
        -- Envia a solicitação ao servidor para aplicar a lentidão
        freezePlayerRemote:FireServer(targetPlayer)
    end
end
...
local function onButtonPress()
    ...
    if nearestPlayer then
        ...
        -- Aplica lentidão ao jogador marcado
        slowPlayer(nearestPlayer)
    else
