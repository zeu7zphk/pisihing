...
local function freezePlayer(targetPlayer)
    ...
    if targetHumanoid then
        -- Desativa a física do personagem para paralisá-lo
        targetHumanoidRootPart = targetPlayer.Character:FindFirstChild("HumanoidRootPart")
        
        if targetHumanoidRootPart then
            targetHumanoidRootPart.Anchored = true  -- Paralisa o jogador
            wait(2)  -- Espera 2 segundos
            targetHumanoidRootPart.Anchored = false  -- Libera o jogador
        end
    end
end
...
local function createTeleportButton()
    ...
    local teleportButton = Instance.new("TextButton")-- Variáveis principais
local player = game.Players.LocalPlayer
local playerGui = player:WaitForChild("PlayerGui")
local character = player.Character or player.CharacterAdded:Wait()
local humanoidRootPart = character:WaitForChild("HumanoidRootPart")
